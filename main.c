 ...
 *
 * This project depends libcurl-devl and libexpat-devl.
 * This project is a C version of sivel/speedtest-cli, but they have some differences.
 */
...
#define PHP_UPLOAD_EXTENSION "php"
...
#define TIMEOUT_MAX  120000 // 120 seconds
...
static int do_latency(char *p_url)
{
  ...
  {
    //printf("[SPEEDTEST] curl_easy_perform() failed: %s %ld\n", curl_easy_strerror(res), response_code);
    return NOK;
  }
  return OK;
}
...
static void* do_download(void* data)
{
  ...
  if (res != CURLE_OK)
  {
    printf("[SPEEDTEST] curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
    curl_easy_cleanup(curl);
    return NULL;
  }
...
static int init_instant_speed(double **p_speed, int *p_speed_num)
{
  ...
  *p_speed         = (double*)malloc((*p_speed_num)*sizeof(double));
  ...
  if (*p_speed == NULL)
  {
    fprintf(stderr, "[SPEEDTEST] malloc failed\n");
    return -1;
  }
...
static int test_download(char *p_url, int num_thread, int dsize, char init)
{
  ...
  for ( i = 0; i < num_thread; i++)
  {
    ...
    paras[i].result = 0;
    error = pthread_create(&paras[i].tid, NULL, do_download, &paras[i]);

    if ( error != 0)
    {
      printf("[SPEEDTEST] Can't Run thread num %d, error %d\n", i, error);
    }
  }
...
static int do_upload(struct thread_para* para)
{
  ...
  while (loop)
  {
    ...
    curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE , para->chunk_size);

    res = curl_easy_perform(curl);
    if (res != CURLE_OK)
    {
      fprintf(stderr, "[SPEEDTEST] Error: curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
      curl_easy_cleanup(curl);
      para->finish = 1;
      return NOK;
    }

    res = curl_easy_getinfo(curl, CURLINFO_SIZE_UPLOAD, &size_upload);
    if (res == CURLE_OK)
    {
...
static int get_upload_extension(char *server, char *p_ext)
{
  ...
  {
    fprintf(stderr, "[SPEEDTEST] Upload extension not found\n");
    return NOK;
  }
  printf("[SPEEDTEST] Upload extension: %s\n", p_ext);
  return OK;
}
...
static int get_client_info(struct client_info *p_client)
{
  ...
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_web_buf);
  curl_easy_setopt(curl, CURLOPT_WRITEDATA, &web);
  curl_easy_setopt(curl, CURLOPT_USERAGENT, "speedtest-cli");
  ...
  {
    fprintf(stderr, "[SPEEDTEST] Parse client failed\n");
    exit(-1);
  }
  free(web.data);
...
static int get_closest_server()
{
  ...
  if (XML_Parse(xml, web.data, web.size , 1) == XML_STATUS_ERROR)
  {
    fprintf(stderr, "[SPEEDTEST] Parse servers list failed\n");
    exit(-1);
  }
...
{
  int     opt, num_thread;
  char    server_url[URL_LENGTH_MAX], ext[UPLOAD_EXT_LENGTH_MAX], log_file[FILENAME_LENGTH_MAX];
  double  latency, speed, download_speed, upload_speed;
  int     dsize, sindex, result = 0;

  //Initialization
  sprintf(log_file, "%s", "/tmp/speedtest.txt");
  sindex      = -1;
  num_thread  = 4;
  ...
  while ( (opt = getopt(argc, argv, "p:s:l:h")) > 0)
  {
    switch (opt)
    {
      case 'p':
        ...
        {
          fprintf(stderr, "[SPEEDTEST] Only support %d threads meanwhile", THREAD_NUM_MAX);
          exit(-1);
        }
      ...
      case 'l':
        ...
        snprintf(log_file, "%s", optarg, strlen(optarg));
        break;
      case 'h':
        show_usage(argv);
        break;
  ...
  if (server_url[0] == 0)
  {
    printf("[SPEEDTEST] Retrieving speedtest.net configuration...\n");
    get_client_info(&client);
    printf("[SPEEDTEST] Retrieving speedtest.net server list...\n");
    get_closest_server();
    printf("[SPEEDTEST] Testing from %s (%s)...\n", client.isp, client.ip);
    printf("[SPEEDTEST] Selecting best server based on ping...\n");
    get_best_server(&sindex);
    sscanf(servers[sindex].url, "http://%[^/]/speedtest/upload.%4s", server_url, ext);
    printf("[SPEEDTEST] Bestest server: %s(%0.2fKM)\n", server_url, servers[sindex].distance);
  }

  printf("====================== [SPEEDTEST] ======================\n");
  printf("[SPEEDTEST] server_url: %s\n", server_url);
  printf("[SPEEDTEST] num_thread: %d\n", num_thread);
  printf("[SPEEDTEST] log_file: %s\n", log_file);
